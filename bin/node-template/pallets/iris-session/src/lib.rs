...
impl<T: Config> Pallet<T> {
    ...
    fn handle_data_requests() -> Result<(), Error<T>> {
        let data_queue = <pallet_iris_assets::Pallet<T>>::DataQueue::<T>::get();
        ...
        for cmd in data_queue.into_iter() {
            match cmd {
                ...
                DataCommand::CatBytes(owner, cid, recipient) => {
						...
						<pallet_iris_assets::Pallet<T>>::AssetClassOwnership::<T>::contains_key(
							owner.clone(), cid.clone()
						), 
						<pallet_iris_assets::Pallet<T>>::Error::<T>::NoSuchOwnedContent
					);

                    let asset_id = <pallet_iris_assets::Pallet<T>>::AssetClassOwnership::<T>::get(owner.clone(), cid.clone());
 					// TODO: Don't want to also depend on the assets pallet here, need to fold this into the iris-assets runtime
                    ...
                    ensure!(balance_primitive != Some(0), <pallet_iris_assets::Pallet<T>>::Error::<T>::InsufficientBalance);
                    match Self::ipfs_request(IpfsRequest::CatBytes(cid.clone()), deadline) {
                        Ok(IpfsResponse::CatBytes(data)) => {
